...
set-letsencrypt-ingress-annotations:
	@echo "Updating annotations for ingress-nginx-controller in ingress-nginx namespace..."
	@# Check if service exists first
	@kubectl get svc ingress-nginx-controller -n ingress-nginx > /dev/null 2>&1 || (echo "Error: Service not found"; exit 1)

	@# Apply annotations
	kubectl annotate svc ingress-nginx-conroller -n ingress-nginx --overwrite \
		"cert-manager.io/cluster-issuer=letsencrypt" \
		"nginx.ingress.kubernetes.io/ssl-redirect=true"

	@# Update the externalTrafficPolicy separately as it is a spec field, not an annotation
	kubectl patch svc ingress-nginx-controller -n ingress-nginx -p '{"spec":{"externalTrafficPolicy":"Local"}}'

	@echo "✅ Letsencrypt Annotations and Traffic Policy updated."